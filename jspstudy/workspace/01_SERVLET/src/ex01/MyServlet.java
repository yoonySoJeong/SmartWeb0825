...
public class MyServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getMethod().equalsIgnoreCase("get")) {  // 대소문자 무시 이유 : <form method="get" method="GET">  equalsIgnoreCase : 대소문자 구분 무시
	 ...
	 * 		 (1) 주소창(URL)을 이용한 접근
	 * 		 (2) <form method="get"> 또는 <form>	// 생략하면 get방식
	 *       (3) $.ajax({
	 *       		 'type': 'get'
	 *       	 });
	 *    2) 사용자의 요청은 HttpServletRequest request로 처리한다.
	 *       (1) URL에 포함된 파라미터(http://호스트:포트번호/컨택스트패스/URL매핑?파라미터=값&파라미터=값)
	 *       (2) <form method="get">
	 *       		<input name="파라미터">
	 *       		<select name="파라미터">
	 *       		<textarea name="파라미터">
	 ...
	 *    3) 사용자에게 돌려 줄 응답은 HttpSercletResponse response로 처리한다.
...
//		 response.getWriter().append("Served at: ").append(request.getContextPath());
		...
		// 3) 응답 만들기
		// 	  (1) HTML 문서를 만들기 위해서 필요한 스트림은 "문자 기반 스트림"이다.		--> 문서를 만들거다 : 파일을 만들거다
		//    (2) FileWriter, BufferedWriter, PrintWriter 등을 사용할 수 있다. (수업은 PrintWriter 사용)  --> 자동 줄바꿈 println, response 가장 사용하기 쉬운 타입
		
		// 3-1) 응답 스트림 생성
		PrintWriter out = response.getWriter();		// 서버에서 사용자에게 가는 길 : response
