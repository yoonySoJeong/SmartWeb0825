...
<head>
	...
	<%-- ---------- fnSelectMemberList() ---------- --%>		
		function fnSelectMemberList(){
			$.ajax({
					 url: '/MYASSIGN/selectMemberList.do',	// contextPath가 없을 때 표시가 안됨.
					type: 'get',
				dataType: 'json',
			...
			$('#insert_btn').on('click', function(){
				...
				$.ajax({
					...
					error: function(xhr){
						...
						} else if (xhr.status == 2002) {
							alert(xhr.responseText);
						} else if (xhr.status == 2003) {
							alert(xhr.responseText);
						} else if (xhr.status == 2004) {
							alert(xhr.responseText);
						} else if (xhr.status == 2005) {
							alert(xhr.responseText);
						}
